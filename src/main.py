...
class SheetsScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__()
        MDApp.get_running_app().sheet_manager = MDApp.get_running_app().sheet_manager
...
class StatisticsScreen(MDScreen):
    ...
    def update_pie_chart(self):
        ...
        for category in labels:
            values[category] = MDApp.get_running_app().sheet_manager.calculate_sum_filter(filters=[["category", category]])

        data = Series(values).reset_index(name='value').rename(columns={'index': 'category'})
        data['angle'] = data['value']/data['value'].sum() * 2*pi
        data['color'] = Category20c[len(values)]

        p = figure(height=400, title=f"Pie Chart for {target_worksheet.title}", toolbar_location=None, x_range=(-0.5, 1.0))
    ...
    def update_bar_chart(self, start_index):
        ...
        p.vbar_stack(MDApp.get_running_app().PAID_FOR_OPTIONS, x='time', width=0.4, color=HighContrast3, source=option_data, legend_label=MDApp.get_running_app().PAID_FOR_OPTIONS)
...
class SettingsScreen(MDScreen):
    ...
    def __init__(self, **kwargs):
        ...
        self.credential = MDApp.get_running_app().CREDENIAL
...
class CardViewScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        self.edit_control = MDGridLayout(cols=2, rows=1, padding=[10, 10, 10, 10], spacing=[10, 0], size_hint_x=1, size_hint_y=None, height="70px")

        self.edit_control_center = MDAnchorLayout(anchor_x="center", anchor_y="center")
        self.delete_control_center = MDAnchorLayout(anchor_x="center", anchor_y="center", padding=[10, 10], height="50px")

        self.save_button =  MDRaisedButton(text="save", size_hint=[.5, .7], on_release=self.save_button_callback)
        self.cancel_button = MDRectangleFlatButton(text="cancel", size_hint=[.5, .7], on_release=self.cancel_button_callback)
        self.delete_button = MDRaisedButton(md_bg_color="red", text="delete", size_hint=[1, 1], on_release=self.delete_button_callback)
        ...
        self.price_field = MDTextField(mode="rectangle", hint_text="Price", helper_text="Price must be a number", helper_text_mode="on_error", required=True)
    ...
    def update_form(self, delete_disabled=False):
        ...
        if not self.selected_card == None: 
...
class SheetCard(ButtonBehavior, MDBoxLayout): #type: ignore
    def __init__(self, index, data : list, callback=None):
        ...
        self.index = index
        self.data = data
        self.left_widget = CardLeftWidget(category=MDApp.get_running_app().sheet_manager.get_data_by_name(data, "category"),
...
class App(MDApp):
    ...
    def retrieve_saved_config(self):
        ...
        with open(CREADENTIAL_PATH, "r") as file:
            self.CREDENIAL = (file.read().strip())

    def write_config(self, credential, sheet_id, users):
        with open(CREADENTIAL_PATH, "w") as file:
    ...
    def open_error_dialog(self):
        error_dialog = NotificationDialog("[ERROR] A problem arised while trying to connect to google sheet!! Please check your connections or ensure credential settings are correct")
        error_dialog.show()
